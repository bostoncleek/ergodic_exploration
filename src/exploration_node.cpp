...
int main(int argc, char** argv)
{
  ...
  ros::Publisher pose_pub = nh.advertise<geometry_msgs::Twist>("cmd_vel", 1);
  ...
  ErgodicControl ergodic_control(omni, ec_dt, ec_horizon, num_samples, buffer_size, batch_size,
                                 R, Sigma);
  //////////////////////////////////////////////////////////////////////////////
  // dwa
  ...
  const auto acc_lim_x = 1.0;
  const auto acc_lim_y = 1.0;
  const auto acc_lim_th = 2.0;
  ...
  const auto max_rot_vel = 1.0;
  const auto min_rot_vel = -1.0;

  const auto vx_samples = 5;
  const auto vy_samples = 10;
